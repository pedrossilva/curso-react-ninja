...
const checkException = () => expect().to.be.equal('ERROR: EXCEPTION NOT IMPLEMENTED')
...
test('pagination({ total: "abc", activePage: 1 }) should throw an error', () => {
  ...
  try { pagination(params); checkException() } catch (e) { expect(e.message).to.be.equal(result) }
})
